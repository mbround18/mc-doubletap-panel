<template>
  <div id="data-fetching" class="links" v-if="loggedIn">
    <v-btn
      color="primary"
      dark
...
import {Player} from "~/store/whitelist";
...
export default class Whitelist  extends Vue {
  ...
  async getWhitelist() {
    await this.$store.dispatch('whitelist/fetch')
  ...
  async addToWhitelist() {
    ...
    await this.$store.dispatch('whitelist/update', {action: 'add',
      player })
  ...
  async removeFromWhitelist(player: Player) {
    this.isLoading = true;
